...
def get_folder_content_summary(folder_name, config=None):
    ...
    # Get file selection limits from config
    max_notebooks = config.get('file_selection', {}).get('max_notebooks', 3)
    max_notebook_cells = config.get('file_selection', {}).get('max_notebook_cells', 5)
    max_python_files = config.get('file_selection', {}).get('max_python_files', 3)
    max_python_lines = config.get('file_selection', {}).get('max_python_lines', 50)
    max_markdown_files = config.get('file_selection', {}).get('max_markdown_files', 5)
    
    # Calculate estimated token budget (200K tokens max, reserve 30K for prompt template and other overhead)
    max_tokens = 170000
    ...
    other_files = [f for f in selected_files if not (f.endswith('.ipynb') or f.endswith('.py') or f.endswith('.md'))]
    ...
    content_summary += f"Other files: {len([f for f in all_files if not (f.endswith('.ipynb') or f.endswith('.py') or f.endswith('.md'))])}\n\n"
    
    if len(important_files) > 0:
        content_summary += f"Important files selected based on patterns: {len(important_files)}\n\n"
    
    # Warning if many files are skipped
    files_analyzed = len(notebooks[:max_notebooks]) + len(python_files[:max_python_files]) + len(markdown_files)
    if total_files > files_analyzed + 10:  # If we're skipping more than 10 files
        content_summary += f"**Note: Only analyzing {files_analyzed} out of {total_files} files due to size constraints.**\n\n"
    ...
    for notebook_path in notebook_paths[:max_notebooks]:
        ...
        if notebook_content:
            ...
            try:
                ...
                if len(notebook_json.get('cells', [])) > cells_to_include:
                    note = f"*Note: Only showing first {cells_to_include} cells out of {len(notebook_json.get('cells', []))}*\n\n"
                    content_summary += note
                    tokens_used += len(note) // 4
            except:
    ...
    for py_path in python_paths[:max_python_files]:
        ...
        if py_content:
            ...
            if tokens_used + code_tokens > max_tokens:
                ...
                if len(py_lines) > lines_to_include:
                    note = f"*Note: Only showing first {lines_to_include} lines out of {len(py_lines)}*\n\n"
                    content_summary += note
                    tokens_used += len(note) // 4
...
def generate_summary_with_bedrock(folder_name, prompt_template, config):
    ...
    if debug_enabled and print_prompt:
        ...
        debug_log_path = logs_dir / f"debug_{folder_name}.log"
...
def generate_summary_file(folder_name, prompt_template, config):
    ...
    if summary:
        # Create output filename using the full folder name
        output_file = ANALYSIS_DIR / f"{config['output']['file_prefix']}{folder_name}.md"
